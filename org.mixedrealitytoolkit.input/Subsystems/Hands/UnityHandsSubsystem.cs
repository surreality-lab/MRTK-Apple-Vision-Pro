        ...
        private class UnityHandContainer : HandDataContainer
        {
            ...
            private void TryCalculateEntireHand()
            {
                using (TryCalculateEntireHandPerfMarker.Auto())
                {
                    ...
                    // Skip the palm for Vision Pro reasons...
                    for (int i = 1; i < (int)TrackedHandJoint.TotalJoints; i++)
                    {
                        FullQueryValid &= TryUpdateJoint(i, hand.GetJoint(TrackedHandJointIndexToXRHandJointID[i]), origin);
                    }

                    if (FullQueryValid) {
                        // We estimate the palm via middle metacarpal
                        UnityEngine.Vector3 midToIndex = HandJoints[(int)TrackedHandJoint.IndexProximal].Position -  HandJoints[(int)TrackedHandJoint.MiddleMetacarpal].Position;
                        UnityEngine.Vector3 midToMid = HandJoints[(int)TrackedHandJoint.MiddleProximal].Position -  HandJoints[(int)TrackedHandJoint.MiddleMetacarpal].Position;
                        UnityEngine.Vector3 midToRing = HandJoints[(int)TrackedHandJoint.RingProximal].Position - HandJoints[(int)TrackedHandJoint.MiddleMetacarpal].Position;

                        UnityEngine.Vector3 palmPositionEstimate = (((midToIndex * 0.5f) + (midToMid * 0.5f) + (midToRing * 0.5f)) * 0.3333f) + HandJoints[(int)TrackedHandJoint.MiddleMetacarpal].Position;


                        HandJoints[0] = new HandJointPose(
                            palmPositionEstimate,
                            HandJoints[(int)TrackedHandJoint.MiddleMetacarpal].Rotation,
            ...
            private bool TryUpdateJoint(int jointIndex, XRHandJoint xrHandJoint, Transform playspaceTransform)
            {
                using (TryUpdateJointPerfMarker.Auto())
                {
                    ...
                    {
                        // Check if we tried to get the palm
                        if (jointIndex == (int)TrackedHandJoint.Palm) {
                            // Ugh, let's estimate
                            bool gotSubposes = true;
                            gotSubposes &= hand.GetJoint(XRHandJointID.IndexProximal).TryGetPose(out Pose indexProx);
                            gotSubposes &= hand.GetJoint(XRHandJointID.MiddleProximal).TryGetPose(out Pose middleProx);
                            gotSubposes &= hand.GetJoint(XRHandJointID.RingProximal).TryGetPose(out Pose ringProx);
                            gotSubposes &= hand.GetJoint(XRHandJointID.MiddleMetacarpal).TryGetPose(out Pose middleMeta);
                            // Can't even estimate if we can't get subjoints
                            if (!gotSubposes) {
                                return false;
                            }
                            UnityEngine.Vector3 midToIndex = indexProx.position - middleMeta.position;
                            UnityEngine.Vector3 midToMid = middleProx.position - middleMeta.position;
                            UnityEngine.Vector3 midToRing = ringProx.position - middleMeta.position;
                            // Estimate palm position
                            UnityEngine.Vector3 palmPositionEstimate = (((midToIndex * 0.5f) + (midToMid * 0.5f) + (midToRing * 0.5f)) * 0.3333f) + middleMeta.position;
                            // Construct estimated pose for the palm
                            pose = new Pose(palmPositionEstimate, middleMeta.rotation);
                        } else {
                            return false;
                        }
                    }
